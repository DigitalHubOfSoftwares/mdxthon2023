...
const SideBarContactUsForm = () => {

	//Open Drawer
	const showContact = () =>{
		const $targetEl: HTMLElement | null = document.getElementById('drawer-contact');

		const options: DrawerOptions = {
			backdrop: true,
			bodyScrolling: false,
			backdropClasses: 'backdrop-blur-sm bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-30',	
		}
	...
	const closeContact = () =>{
		...
		const options: DrawerOptions = {
			...
			bodyScrolling: true,
			onHide: () => {
				// Find all elements with drawer-backdrop attribute
				const elements = document.querySelectorAll('[drawer-backdrop]');
	...
	return (
		<div>
			<div className="text-center">
				<button className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800" type="button" onClick={showContact} data-drawer-target="drawer-contact" data-drawer-show="drawer-contact" aria-controls="drawer-contact" data-drawer-backdrop="false">
					Show contact form
				</button>
			</div>
			<div id="drawer-contact" className="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-80 dark:bg-gray-800" tabIndex={-1} aria-labelledby="drawer-contact-label">
				...
				<button type="button" data-drawer-hide="drawer-contact" aria-controls="drawer-contact" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" onClick={closeContact} >
...
export default SideBarContactUsForm;